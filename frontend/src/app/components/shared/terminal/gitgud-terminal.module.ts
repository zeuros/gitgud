 ...
 */
@Component({
    selector: 'gitgud-terminal',
    template: `
        <div [ngClass]="'p-terminal gitgud-terminal p-component'" [ngStyle]="style ?? {}" [class]="styleClass ?? ''" (click)="focus(in)">
    ...
    styleUrls: ['/node_modules/primeng/resources/components/terminal/terminal.css'],
    host: {
        class: 'p-element'
    }
})
export class Terminal implements AfterViewInit, AfterViewChecked, OnDestroy {
    ...
    ngAfterViewInit() {
        this.container = DomHandler.find(this.el.nativeElement, '.gitgud-terminal')[0];
    }
...
export class GitgudTerminalModule {}