...
export class FileDiffService {
  ...
  getCommitDiff = (file: FileChange, commitish: string, hideWhitespaceInDiff = false) =>
    this.gitRepositoryService
      ...
      .pipe(map(r => buildDiff(r, file, commitish)));

  // private buildDiff(rawDiffStr: string, file: FileChange, oldestCommitish: string, lineEndingsChange?: LineEndingsChange) {
  //   const diff = this.diffFromRawDiffOutput(rawDiffStr);
  //
  //   if (isStringTooLarge(rawDiffStr) || isDiffTooLarge(diff)) {
  ...
  //
  //
  // private diffFromRawDiffOutput = (output: string): IRawDiff => {
