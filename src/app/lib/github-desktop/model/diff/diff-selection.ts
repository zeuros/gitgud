...
function typeMatchesSelection(
  ...
  if (selectionType == DiffSelectionType.All) return selected;
  else if (selectionType == DiffSelectionType.None) return !selected;
  else if (selectionType == DiffSelectionType.Partial) return false;

  return throwEx(`Unknown selection type ${selectionType}`);
}
...
export class DiffSelection {
  ...
  public static fromInitialSelection(
    ...
    if (
      ...
      throw Error('Can only instantiate a DiffSelection with All or None as the initial selection');
    }
  ...
  isSelected = (lineIndex: number) => {

    const lineIsDivergent = !!this.divergingLines && this.divergingLines.has(lineIndex)

    if (this.defaultSelectionType === DiffSelectionType.All) return !lineIsDivergent
    else if (this.defaultSelectionType === DiffSelectionType.None) return lineIsDivergent

    throw Error(`Unknown base selection type ${this.defaultSelectionType}`);
  }
