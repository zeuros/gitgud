...
import {APIError, getUserAgent, HTTPMethod, parsedResponse, request, urlWithQueryString,} from './http'
import {uuid} from './uuid'
import {GitProtocol} from './remote-parsing'
import {getEndpointVersion, isDotCom, isGHE, updateEndpointVersion,} from './endpoint-capabilities'
import {clearCertificateErrorSuppressionFor, suppressCertificateErrorFor,} from './suppress-certificate-error'
...
export class API {
  ...
  public async getAliveDesktopChannel(): Promise<IAPIAliveSignedChannel | null> {
    try {
      ...
      console.warn(`Alive channel request failed: ${e}`)
      return null
    }
  }
  ...
  public async getAliveWebSocketURL(): Promise<string | null> {
    try {
      ...
      console.warn(`Alive web socket request failed: ${e}`)
      throw e
    }
  }
  ...
  public async fetchRepository(
    ...
    try {
      ...
      if (response.status === HttpStatusCode.NotFound) {
        console.warn(`fetchRepository: '${owner}/${name}' returned a 404`)
        return null
      }
      return await parsedResponse<IAPIFullRepository>(response)
    } catch (e) {
      console.warn(`fetchRepository: an error occurred for '${owner}/${name}'`, e)
      return null
    }
  }
  ...
  public async fetchAccount(): Promise<IAPIFullIdentity> {
    try {
      ...
      console.warn(`fetchAccount: failed with endpoint ${this.endpoint}`, e)
      throw e
    }
  }
  ...
  public async fetchEmails(): Promise<ReadonlyArray<IAPIEmail>> {
    try {
      ...
      console.warn(`fetchEmails: failed with endpoint ${this.endpoint}`, e)
      return []
    }
  }
  ...
  public async fetchOrgs(): Promise<ReadonlyArray<IAPIOrganization>> {
    try {
      ...
      console.warn(`fetchOrgs: failed with endpoint ${this.endpoint}`, e)
      return []
    }
  }
  ...
  public async createRepository(
    ...
    try {
      ...
      console.error(`createRepository: failed with endpoint ${this.endpoint}`, e)
      throw new Error(
  ...
  public async fetchIssues(
    ...
    try {
      ...
      console.warn(`fetchIssues: failed for repository ${owner}/${name}`, e)
      throw e
    }
  }
  ...
  public async fetchAllOpenPullRequests(owner: string, name: string) {
    ...
    try {
      ...
      console.warn(`failed fetching open PRs for repository ${owner}/${name}`, e)
      throw e
    }
  }
  ...
  public async fetchUpdatedPullRequests(
    ...
    try {
      ...
      console.warn(`failed fetching updated PRs for repository ${owner}/${name}`, e)
      throw e
    }
  }
  ...
  public async fetchPullRequest(owner: string, name: string, prNumber: string) {
    try {
      ...
      console.warn(`failed fetching PR for ${owner}/${name}/pulls/${prNumber}`, e)
      throw e
    }
  }
  ...
  public async fetchCombinedRefStatus(
    ...
    try {
      ...
      console.debug(`Failed fetching check runs for ref ${ref} (${owner}/${name})`, err)
      return null
    }
  }
  ...
  public async getAvatarToken() {
    return this.request('GET', `/desktop/avatar-token`)
      ...
      .catch(err => {
        console.debug(`Failed to load avatar token`, err)
        return null
      })
  }
  ...
  private async fetchAll<T>(path: string, options?: IFetchAllOptions<T>) {
    ...
    do {
      ...
      if (opts.suppressErrors !== false && !response.ok) {
        console.warn(`fetchAll: '${path}' returned a ${response.status}`)
        return buf
      }
  ...
  public async getFetchPollInterval(
    ...
    try {
      ...
      console.warn(`getFetchPollInterval: failed for ${owner}/${name}`, e)
      return null
    }
  }
  ...
  public async fetchMentionables(
    ...
    try {
      ...
      if (response.status === HttpStatusCode.NotFound) {
        console.warn(`fetchMentionables: '${path}' returned a 404`)
        return null
      }
      ...
      console.warn(`fetchMentionables: failed for ${owner}/${name}`, e)
      return null
    }
  }
  ...
  public async fetchUser(login: string): Promise<IAPIFullIdentity | null> {
    try {
      ...
      console.warn(`fetchUser: failed with endpoint ${this.endpoint}`, e)
      throw e
    }
  }
}
...
export async function deleteToken(account: Account) {
  try {
    ...
    console.error(`deleteToken: failed with endpoint ${account.endpoint}`, e)
    return false
  }
}
...
export async function fetchUser(
  ...
  try {
    ...
    console.warn(`fetchUser: failed with endpoint ${endpoint}`, e)
    throw e
  }
}
...
export async function requestOAuthToken(
  ...
  try {
    ...
    console.warn(`requestOAuthToken: failed with endpoint ${endpoint}`, e)
    return null
  }
}
...
export async function isGitHubHost(url: string) {
  ...
  try {
    ...
    console.debug(`isGitHubHost: failed with endpoint ${endpoint}`, e)
    return undefined
  } finally {
