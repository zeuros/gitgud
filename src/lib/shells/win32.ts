...
async function findHyper(): Promise<string | null> {
  ...
  if (first.type === RegistryValueType.REG_SZ) {
    ...
    if (path == null) {
      ...
      console.debug(`[Hyper] registry entry found but does not exist at '${path}'`)
    }
  }
...
async function findCygwin(): Promise<string | null> {
  ...
  if (
    ...
    if (await pathExists(path)) {
      ...
      console.debug(`[Cygwin] registry entry found but does not exist at '${path}'`)
    }
  }
...
async function findWSL(): Promise<string | null> {
  ...
  if (!(await pathExists(wslPath))) {
    console.debug(`[WSL] wsl.exe does not exist at '${wslPath}'`)
    return null
  }
  ...
  try {
    ...
    console.error(`[WSL] unhandled error when invoking 'wsl /list'`, err)
  }
  return null
}
...
export function launch(
  ...
  switch (shell) {
    ...
    case Shell.Hyper:
      ...
      console.info(`launching ${shell} at path: ${hyperPath}`)
    ...
    case Shell.Alacritty:
      ...
      console.info(`launching ${shell} at path: ${alacrittyPath}`)
    ...
    case Shell.GitBash:
      ...
      console.info(`launching ${shell} at path: ${gitBashPath}`)
    ...
    case Shell.Cygwin:
      ...
      console.info(`launching ${shell} at path: ${cygwinPath}`)
      return spawn(
        cygwinPath,
...
export function launchCustomShell(
  ...
  console.info(`launching custom shell at path: ${customShell.path}`)
