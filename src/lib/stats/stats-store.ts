...
export class StatsStore implements IStatsStore {
  ...
  public constructor(
    ...
    window.addEventListener('unhandledrejection', async () => {
      try {
        ...
        console.error(`Failed recording unhandled rejection`, err)
      }
    })
  }
  ...
  public async reportStats(
    ...
    try {
      ...
      console.info('Stats reported.')

      await this.clearDailyStats()
      setNumber(LastDailyStatsReportKey, now)
    } catch (e) {
      console.error('Error reporting stats:', e)
    }
  }
  ...
  private async getDailyStats(
    ...
    const launchedFromApplicationsFolder = isPlatformDarwin()
  ...
  public async recordOperationUndone(
    ...
    switch (kind) {
      ...
      case MultiCommitOperationKind.Rebase:
      case MultiCommitOperationKind.Merge:
        console.error(`[recordOperationUndone] - Operation not supported: ${kind}`)
        return
      default:
        return assertNever(kind, `Unknown operation kind of ${kind}.`)
    }
  ...
  private async sendOptInStatusPing(
    ...
    try {
      ...
      console.info(`Opt ${direction} reported.`)
    } catch (e) {
      console.error(`Error reporting opt ${direction}:`, e)
    }
  }
}
