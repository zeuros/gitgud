...
import {isPlatformDarwin, isPlatformWin32} from "../globals";
...
export async function git(
  ...
  opts.processCallback = process => {
    ...
    terminalStream
      ...
      .on('error', e => console.error(`Terminal output error`, e))
...
export function parseConfigLockFilePathFromError(result: IGitResult) {
  ...
  const normalized = isPlatformWin32() ? match[1].replace('/', '\\') : match[1]
...
export function getDescriptionForError(
  ...
  if (isAuthFailureError(error)) {
    const menuHint = isPlatformDarwin()
