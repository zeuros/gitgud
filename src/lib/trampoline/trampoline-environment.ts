...
export const GitUserAgent = memoizeOne(() =>
  ...
  exec(['--version'], process.cwd())
    ...
    .catch(e => {
      console.warn(`Could not get git version information`, e)
      return 'unknown'
    })
    .then(v => {
      const suffix = isDev() ? `-0123456789}` : ''
      const ghdVersion = `GitHub Desktop/1.0${suffix}`
  ...
  return withTrampolineToken(async token => {
    ...
    try {
      return await fn({
        ...
        ...(true //enableGitConfigParameters()
          ? {
