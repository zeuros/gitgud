...
export class AliveStore {
  ...
  private async createSessionForAccount(
    ...
    try {
      ...
      console.error(`Could not get Alive web socket URL for '${account.login}'`, e)
      return null
    }
  ...
  private unsubscribeFromAccount(account: Account) {
    ...
    console.info(`Unubscribed '${account.login}' from Alive channel`)
  }
  ...
  private subscribeToAccount = async (account: Account) => {
    ...
    console.info(`Subscribed '${account.login}' to Alive channel`)
  }
