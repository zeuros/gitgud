...
export class AppStore extends TypedBaseStore<IAppState> {
  ...
  private async getWindowZoomFactor() {
    ...
    // update
    if (zoomFactor !== 1 || !isPlatformWin32()) {
      return zoomFactor
    }
  ...
  private onTokenInvalidated = (endpoint: string, token: string) => {
    ...
    if (account.token && account.token !== token) {
      console.error(`Token for ${endpoint} invalidated but token mismatch`)
      return
    }
  ...
  private async updateCurrentTutorialStep(
    ...
    if (currentStep !== this.currentOnboardingTutorialStep) {
      ...
      console.info(`Current tutorial step is now ${currentStep}`)
  ...
  public async loadEmoji() {
    ...
    readEmoji(rootDir)
      ...
      .catch(err => {
        console.warn(`Unexpected issue when trying to read emoji into memory`, err)
      })
  }
  ...
  public async loadInitialState() {
    ...
    accounts.forEach(a => {
      console.info(`[AppStore] found account: ${a.login} (${a.name})`)
    })
    ...
    ).catch(e => console.error('Failed resolving current editor at startup', e))
  ...
  private async refreshAfterCheckout(
    ...
    this.updateCheckoutProgress(repository, {
      kind: 'checkout',
      title: `Refreshing ${isPlatformDarwin() ? 'Repository' : 'repository'}`,
      ...
      if (branch.type === BranchType.Remote) {
        ...
        const remote =
          ...
          (await getRemoteURL(repository, remoteName)
            ...
            .catch(e => console.debug(`Could not get remote URL`, e)))
    ...
    return this.withPushPullFetch(repository, async () => {
      ...
      await gitStore.performFailableOperation(
        async () => {
          ...
          const refreshTitle = isPlatformDarwin()
        ...
        try {
          ...
          if (pullSucceeded) {
            ...
            await updateRemoteHEAD(repository, remote, false).catch(e =>
              console.error('Failed updating remote HEAD', e)
            )
          }
  ...
  private async fastForwardBranches(repository: Repository) {
    try {
      ...
      console.error('Branch fast-forwarding failed', e)
    }
  }
  ...
  public async _discardChanges(
    ...
    try {
      ...
      if (!(error instanceof DiscardChangesError)) {
        console.error('Failed discarding changes', error)
      }
  ...
  public async _mergeBranch(
    ...
    if (
      ...
      console.error('[mergeBranch] - Not in merge operation state')
      return
    }
  ...
  public async _editGlobalGitConfig() {
    await getGlobalConfigPath()
      ...
      .catch(e => console.error('Could not open global Git config for editing', e))
  }
  ...
  public async _addRepositories(
    ...
    for (const path of paths) {
      ...
      if (repositoryType.kind === 'regular') {
        ...
        console.info(`[AppStore] adding repository at ${validatedPath} to store`)
    ...
    try {
      if (moveToTrash) {
        try {
          ...
          console.error('Failed moving repository to trash', error)
  ...
  public async _findPullRequestBranch(
    ...
    if (existingBranch === undefined) {
      try {
        ...
        console.error(`Failed fetching remote ${remote?.name}`, e)
      }
    }
  ...
  public async _cherryPick(
    ...
    if (commits.length === 0) {
      console.error('[_cherryPick] - Unable to cherry-pick. No commits provided.')
  ...
  public async _reorderCommits(
    ...
    if (commitsToReorder.length === 0) {
      console.error('[_reorder] - Unable to reorder. No commits provided.')
  ...
  public async _squash(
    ...
    if (toSquash.length === 0) {
      console.error('[_squash] - Unable to squash. No commits provided.')
      return RebaseResult.Error
    }
  ...
  public async _undoMultiCommitOperation(
    ...
    if (undoSha === null) {
      console.error('[_undoMultiCommitOperation] - Could not determine undo sha')
      return false
    }
