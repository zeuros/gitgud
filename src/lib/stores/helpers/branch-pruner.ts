...
export class BranchPruner {
  ...
  private async pruneLocalBranches(
    ...
    if (
      ...
      console.info(`[BranchPruner] Last prune took place ${timeAgo} - skipping`)
      return
    }
    ...
    if (mergedBranches.size === 0) {
      console.info('[BranchPruner] No branches to prune.')
      return
    }
    ...
    for (const branchCanonicalRef of branchesReadyForPruning) {
      ...
      if (options.deleteBranch) {
        ...
        console.info(`[BranchPruner] Branch '${branchName}' marked for deletion`)
      }
    }
    this.onPruneCompleted(this.repository).catch(e => {
      console.error(`[BranchPruner] Error calling onPruneCompleted`, e)
    })
  }
}
