...
export class SignInStore extends TypedBaseStore<SignInState | null> {
  ...
  public async authenticateWithBrowser() {
    ...
    new Promise<Account>((resolve, reject) => {
      ...
      console.info('[SignInStore] initializing OAuth flow')
      this.setState({
      ...
      .then(account => {
        if (!this.state || this.state.kind !== SignInStep.Authentication) {
          ...
          console.warn('[SignInStore] account resolved but session has changed')
          return
        }

        console.info('[SignInStore] account resolved')
      ...
      .catch(e => {
        ...
        if (
          ...
          console.info('[SignInStore] error with OAuth flow', e)
          this.setState({...this.state, error: e, loading: false})
        } else {
          console.info(`[SignInStore] OAuth error but session has changed: ${e}`)
        }
      })
  }
