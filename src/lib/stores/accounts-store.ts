...
export class AccountsStore extends TypedBaseStore<ReadonlyArray<Account>> {
  ...
  public async addAccount(account: Account): Promise<Account | null> {
    ...
    try {
      ...
      console.error(`Error adding account '${account.login}'`, e)

      if (isPlatformDarwin() && isKeyChainError(e)) {
        this.emitError(
          new Error(
  ...
  private async tryUpdateAccount(account: Account): Promise<Account> {
    try {
      ...
      console.warn(`Error refreshing account '${account.login}'`, e)
      return account
    }
  }
  ...
  public async removeAccount(account: Account): Promise<void> {
    ...
    try {
      ...
      console.error(`Error removing account '${account.login}'`, e)
      this.emitError(e)
      return
    }
  ...
  private async loadFromStore(): Promise<void> {
    ...
    for (const account of rawAccounts) {
      ...
      try {
        ...
        console.error(`Error getting token for '${key}'. Skipping.`, e)
