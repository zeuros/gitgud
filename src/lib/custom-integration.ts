...
export function parseCustomIntegrationArguments(
  ...
  return isPlatformWin32() ? parseCommandLineArgv(args) : stringArgv(args)
}
...
export async function validateCustomIntegrationPath(
  ...
  try {
    ...
    // the app bundle ID)
    if (isPlatformDarwin() && !isExecutableFile && pathStat.isDirectory()) {
...
export async function isValidCustomIntegration(
  ...
  try {
    ...
    console.error('Failed to validate custom integration:', e)
    return false
  }
}
...
export function spawnCustomIntegration(
  ...
  if (isPlatformWin32() && options?.shell) {
    command = `"${command}"`
    args = args.map(a => `"${a}"`)
  }
